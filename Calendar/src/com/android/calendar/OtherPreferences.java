...
public class OtherPreferences extends PreferenceFragment implements OnPreferenceChangeListener {
	...
	public void onCreate(Bundle icicle) {
		...
		mQuietHoursStartDialog = new TimePickerDialog(activity, mQuietHoursStartListener, startHour, startMinute,
				mIs24HourMode);
	 ...
	 * 
	 * @param value
	 *            The corresponding value of which summary to set. If null, the
	 *            default summary will be set, and the value will be set
	 *            accordingly too.
	 */
	private void updateSkipRemindersSummary(String value) {
		if (mSkipReminders != null) {
			// Default to "declined". Must match with
			...
			if (value == null) {
				// Value was not known ahead of time, so the default value will
				// be set.
