...
public class SimpleWeekView extends View {
	...
	protected String[] mGregorianDayNumbers;
	...
	protected int mFocusMonthHollidayColor;
	protected int mOtherMonthHollidayColor;
	...
	public SimpleWeekView(Context context) {
		...
		mFocusMonthHollidayColor = res.getColor(R.color.month_day_number_holliday);
		mOtherMonthHollidayColor = res.getColor(R.color.month_day_number_other_holliday);
	...
	public void setWeekParams(HashMap<String, Integer> params, String tz) {
		...
		mGregorianDayNumbers = new String[mNumCells];
		mFocusDay = new boolean[mNumCells];
		mOddMonth = new boolean[mNumCells];
		mIsHolliday = new boolean[mNumDays];
		...
		mFirstMonth = jDate.month - 1;
		int i = 0;
		if (mShowWeekNum) {
			mDayNumbers[0] = Jalali.persianDigits(Integer.toString(jDate.getWeekNumber()));
			i++;
		}

		long startM = System.currentTimeMillis();
		OccasionHelper.fillWeekHollidays(mIsHolliday, jDate);
		long endM = System.currentTimeMillis();
		// System.out.println("Query for week time: " + (endM - startM) + "ms");

		for (; i < mNumCells; i++) {
			if (jDate.day == 1) {
				mFirstMonth = jDate.month - 1;
			}
			mOddMonth[i] = (jDate.month % 2) == 0;
			if (jDate.month == focusMonth + 1) {
				mFocusDay[i] = true;
			} else {
				mFocusDay[i] = false;
			}
			...
			mDayNumbers[i] = Jalali.persianDigits(Integer.toString(jDate.day));
			mGregorianDayNumbers[i] = Integer.toString(time.monthDay);
		...
		jDate = Jalali.gregorianToJalali(time);
		mLastMonth = jDate.month - 1;
	...
	public Time getDayFromLocation(float x) {
		...
		if (x < dayStart || x > mWidth - (mShowWeekNum ? (mWidth - mPadding * 2) / mNumCells + mPadding : mPadding)) {
			return null;
		}
		// Selection is (x - start) / (pixels/day) == (x -s) * day / pixels
		int dayPosition = (int) ((x - dayStart) * mNumDays / (mWidth - dayStart - (mShowWeekNum ? (mWidth - mPadding * 2)
				/ mNumCells + mPadding
				: mPadding))) - 1;
		dayPosition = 5 - dayPosition;
	...
	protected void drawWeekNums(Canvas canvas) {
		...
		if (mShowWeekNum) {
			...
			int x = (divisor - 1) * (mWidth - mPadding * 2) / divisor + mPadding;
		...
		for (; i < nDays; i++) {
			...
			int j = 7 - i + offset;
			int x = ((2 * (j - 1)) + 1) * (mWidth - mPadding * 2) / (divisor) + mPadding;
	...
	protected void drawDaySeparators(Canvas canvas) {
		...
		if (mShowWeekNum) {
			...
			int x = (mNumCells - 1) * (mWidth - mPadding * 2) / mNumCells + mPadding;
	...
	protected void updateSelectionPositions() {
		if (mHasSelectedDay) {
			...
			selectedPosition = 6 - selectedPosition + offset;
