...
public class MonthWeekEventsView extends SimpleWeekView {
		...
		public void onAnimationEnd(Animator animation) {
			synchronized (this) {
				...
				if (mFadingIn) {
					...
					mTodayAnimator = ObjectAnimator.ofInt(MonthWeekEventsView.this, "animateTodayAlpha", 255, 0);
	...
	public void createDna(ArrayList<Event> unsortedEvents) {
		...
		if (!mShowDetailsInMonth) {
			...
			for (int day = 0; day < numDays; day++) {
				mDayXs[day] = computeDayRightPosition(day + 1) + DNA_WIDTH / 2 + DNA_SIDE_PADDING;

			}

			int top = DAY_SEPARATOR_INNER_WIDTH + DNA_MARGIN + DNA_ALL_DAY_HEIGHT + 1;
			int bottom = mHeight - DNA_MARGIN;
			mDna = Utils.createDNAStrands(mFirstJulianDay, unsortedEvents, top, bottom, DNA_MIN_SEGMENT_HEIGHT, mDayXs,
					getContext());
		}
	}
	...
	public void setEvents(List<ArrayList<Event>> sortedEvents) {
		...
		if (sortedEvents.size() != mNumDays) {
			if (Log.isLoggable(TAG, Log.ERROR)) {
				Log.wtf(TAG, "Events size must be same as days displayed: size=" + sortedEvents.size() + " days="
						+ mNumDays);
			}
			mEvents = null;
			return;
	...
	protected void initView() {
		...
		if (DEBUG_LAYOUT) {
			...
			Log.d("EXTRA", "mMonthNumPaint ascent=" + mMonthNumPaint.ascent() + " descent=" + mMonthNumPaint.descent()
					+ " int height=" + mMonthNumHeight);
			Log.d("EXTRA", "mEventPaint ascent=" + mEventPaint.ascent() + " descent=" + mEventPaint.descent()
					+ " int height=" + mEventHeight + " int ascent=" + mEventAscentHeight);
			Log.d("EXTRA",
					"mEventExtrasPaint ascent=" + mEventExtrasPaint.ascent() + " descent="
							+ mEventExtrasPaint.descent() + " int height=" + mExtrasHeight);
	...
	public void setWeekParams(HashMap<String, Integer> params, String tz) {
		...
		if (params.containsKey(VIEW_PARAMS_ANIMATE_TODAY) && mHasToday) {
			synchronized (mAnimatorListener) {
				...
				mTodayAnimator = ObjectAnimator.ofInt(this, "animateTodayAlpha", Math.max(mAnimateTodayAlpha, 80), 255);
	...
	protected void drawToday(Canvas canvas) {
		...
		r.right = computeDayRightPosition(mTodayIndex) - (int) Math.ceil(TODAY_HIGHLIGHT_WIDTH / 2.0f);
		r.left = computeDayRightPosition(mTodayIndex + 1) + (TODAY_HIGHLIGHT_WIDTH / 2);
	...
	protected void drawDaySeparators(Canvas canvas) {
		...
		while (i < count) {
			int x = computeDayRightPosition(i / 4 - wkNumOffset);
			lines[i++] = x;
			lines[i++] = y0;
			lines[i++] = x;
			lines[i++] = y1;
	...
	protected void drawBackground(Canvas canvas) {
		...
		if (!mOddMonth[i]) {
			while (++i < mOddMonth.length && !mOddMonth[i])
				;
			r.left = computeDayRightPosition(i - offset);
			r.right = computeDayRightPosition(0);
			...
			// compute left edge for i, set up r, draw
			r.left = 0;
			r.right = computeDayRightPosition(i - offset);
			p.setColor(mMonthBGOtherColor);
			canvas.drawRect(r, p);
		}
		if (mHasToday) {
			p.setColor(mMonthBGTodayColor);
			r.left = computeDayRightPosition(mTodayIndex + 1);
			r.right = computeDayRightPosition(mTodayIndex);
			canvas.drawRect(r, p);
		}
	}
	...
	private void drawClick(Canvas canvas) {
		if (mClickedDayIndex != -1) {
			...
			r.left = computeDayRightPosition(mClickedDayIndex + 1);
			r.right = computeDayRightPosition(mClickedDayIndex);
			r.top = DAY_SEPARATOR_INNER_WIDTH;
			r.bottom = mHeight;
			canvas.drawRect(r, p);
			p.setAlpha(alpha);
	...
	protected void drawWeekNums(Canvas canvas) {
		...
		if (mShowWeekNum) {
			x = mWidth - (SPACING_WEEK_NUMBER + mPadding) + SIDE_PADDING_WEEK_NUMBER + mPadding;
		...
		long milis = DateTimeUtils.fromJulianDay(julianMonday);
		time.set(milis);
		...
		for (; i < numCount; i++) {
			...
			boolean isHolliday = mIsHolliday[mShowWeekNum ? i - 1 : i];
			if (isHolliday) {
				mMonthNumPaint.setColor(isFocusMonth ? mFocusMonthHollidayColor : mOtherMonthHollidayColor);
			} else if (!(mHasToday && todayIndex == i)) {
				mMonthNumPaint.setColor(isFocusMonth ? mMonthNumColor : mMonthNumOtherColor);
			}
			x = computeDayRightPosition(i - offset - 1) - (SIDE_PADDING_MONTH_NUMBER);
			canvas.drawText(mDayNumbers[i], x, y, mMonthNumPaint);
			if (Utils.showGregorian()) {
				mMonthNumPaint.setTextSize(TEXT_SIZE_MONTH_NUMBER * 0.7f);
				canvas.drawText(mGregorianDayNumbers[i], x, mHeight - mMonthNumAscentHeight, mMonthNumPaint);
				mMonthNumPaint.setTextSize(TEXT_SIZE_MONTH_NUMBER);
			}
			if (isBold) {
			...
			// LunarUtils.FORMAT_LUNAR_SHORT
			// | LunarUtils.FORMAT_MULTI_FESTIVAL, false, infos);
			// if (infos.size() > 0) {
	...
	protected void drawEvents(Canvas canvas) {
		...
		for (ArrayList<Event> eventDay : mEvents) {
			...
			int ySquare;
			int xSquare = computeDayRightPosition(day + 1) + SIDE_PADDING_MONTH_NUMBER + 1;
			int rightEdge = computeDayRightPosition(day);
	 ...
	 */
	protected int drawEvent(Canvas canvas, Event event, int x, int y, int rightEdge, boolean moreEvents,
		...
		int reservedSpace = EVENT_BOTTOM_PADDING; // leave a bit of room at the
													// bottom
		if (moreEvents) {
			...
			// is expected
			// to be <= the height of an event line, so we won't show "+1" when
			// we could be
			// showing the event.)
		...
		if (allDay) {
			// We shift the render offset "inward", because drawRect with a
			// stroke width greater
			// than 1 draws outside the specified bounds. (We don't adjust the
			// left edge, since
		...
		if (showTimes && !allDay) {
			...
			text = DateUtils.formatDateRange(getContext(), mFormatter, event.startMillis, event.endMillis,
					DateUtils.FORMAT_SHOW_TIME | DateUtils.FORMAT_ABBREV_ALL, Utils.getTimeZone(getContext(), null))
					.toString();
			...
			if (strand != null && strand.allDays != null && strand.allDays.length == numDays) {
				for (int i = 0; i < numDays; i++) {
					...
					if (strand.allDays[i] != 0) {
						...
						canvas.drawLine(mDayXs[i] + xOffset, DNA_MARGIN, mDayXs[i] + xOffset, DNA_MARGIN
	...
	public int getDayIndexFromLocation(float x) {
		...
		if (x < dayStart || x > mWidth - (mShowWeekNum ? SPACING_WEEK_NUMBER + mPadding : mPadding)) {
			return -1;
		}
		// Selection is (x - start) / (pixels/day) == (x -s) * day / pixels
		int index = ((int) ((x - dayStart) * mNumDays / (mWidth - dayStart - mPadding)));
		index = mNumDays - index - 1;
		return index;
	}
