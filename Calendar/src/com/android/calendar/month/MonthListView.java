...
public class MonthListView extends ListView {
	...
	// These define the behavior of the fling. Below MIN_VELOCITY_FOR_FLING, do
	// the system fling
	// behavior. Between MIN_VELOCITY_FOR_FLING and
	// MULTIPLE_MONTH_VELOCITY_THRESHOLD, do one month
	// fling. Above MULTIPLE_MONTH_VELOCITY_THRESHOLD, do multiple month flings
	// according to the
	// fling strength. When doing multiple month fling, the velocity is reduced
	// by this threshold
	...
	private final Runnable mTimezoneUpdater = new Runnable() {
		...
		public void run() {
			if (mTempTime != null && mListContext != null) {
				mTempTime.timezone = Utils.getTimeZone(mListContext, mTimezoneUpdater);
			}
		}
	};
	...
	private void doFling(float velocityY) {
		...
		int offset = (viewsToFling > 0) ? -(firstViewHeight - topViewVisiblePart + SimpleDayPickerFragment.LIST_TOP_OFFSET)
