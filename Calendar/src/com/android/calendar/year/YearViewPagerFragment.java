...
public class YearViewPagerFragment extends Fragment implements CalendarController.EventHandler {
	...
	private static final int EPOCH_TIME_YEAR_START = 1348;
	private static final int EPOCH_TIME_YEAR_END = 1415;

	private ViewPager mPager;
	private YearViewPagerAdapter mPagerAdapter;
	private int mCurrentJalaliYear;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		mPager.setOnPageChangeListener(new ViewPager.SimpleOnPageChangeListener() {
			...
			public void onPageSelected(int position) {
				...
				updateTitle(EPOCH_TIME_YEAR_END - mCurrentPage);
			}
		});
	...
	/**
	 * Interacts with the Calendar controller to update the action bar title
	 * with the current year
	 *
	 * @param year
	 *            year the title should be updated to
	 */
	private void updateTitle(int year) {
		JalaliDate startDate = new JalaliDate(year, 1, 1);
		JalaliDate endDate = new JalaliDate(year, 12, Jalali.isJalaliLeapYear(year) ? 30 : 29);
		Time start = Jalali.jalaliToGregorianTime(startDate);
		Time end = Jalali.jalaliToGregorianTime(endDate);
	...
	public void goToYear(int year) {
		if (mPager != null) {
			mPager.setCurrentItem(EPOCH_TIME_YEAR_END - year);
		}
	}

	@Override
	public long getSupportedEventTypes() {
		// support action bar navigation to the current time
		// ("current time button")
	...
	public void handleEvent(EventInfo event) {
		...
		if (event.eventType == EventType.GO_TO) {
			goToYear(Jalali.gregorianToJalali(event.selectedTime).year);
		}
	}

	@Override
	public void eventsChanged() {
		// nothing to do as this view doesn't depend on the underlying events
		// database
	}
	...
	public static class YearViewFragment extends Fragment {
		...
		static YearViewFragment newInstance(int yearOffset) {
			...
			args.putInt("year", EPOCH_TIME_YEAR_END - yearOffset);
			f.setArguments(args);
		...
		public void onCreate(Bundle savedInstanceState) {
			...
			mYear = getArguments() != null ? getArguments().getInt("year") : EPOCH_TIME_YEAR_START;
		...
		public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
			...
			mGridView.setRotationY(180f);

			// we need to wait till the gridview is laid out
			// the adapter drawing the year view depends on the dimensions of
			// the grid view to
			// figure out the layout and size of each of the months and its
			// parameters
			mGridView.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
				...
				public void onGlobalLayout() {
					...
					// prepare adapter and supply the dimensions it has to work
					// with
		...
		/**
		 * A GridView with a custom scroll listener that is tied to the display
		 * status of FAB
		 */
		public static class CustomGridView extends GridView {
			...
			/*
			 * Overiding method to implement a scroll gesture listener which is
			 * interested in vertical movements to show or hide FAB
			 */
			@Override
			...
			/**
			 * Expects the input MotionEvent to be a move event. This function
			 * determines if there is enough vertical movement beyond the
			 * defined threshold. If there is sufficient movement along the
			 * vertical, the FAB is shown or hidden based on the direction of
			 * movement (up or down)
			 *
			 * @param ev
			 *            event to parse
			...
			private void processNewMoveEvent(MotionEvent ev) {
				...
				if (Math.abs(distanceMoved) > mTouchSlop) {
					if (distanceMoved < 0) {
						...
						mController.sendEvent(this, EventType.SHOW_FAB, null, null, null, 0, 0, 0, null, null);
					} else {
						// scrolle up so hide FAB
						mController.sendEvent(this, EventType.HIDE_FAB, null, null, null, 0, 0, 0, null, null);
