...
public class YearViewAdapter extends BaseAdapter implements View.OnTouchListener {
	...
	// The padding b/w the week rows. The container height sans that taken up by
	// the header
	...
	private int mJalaliYear;
	private int mCurrentJalaliMonth = -1;

	private int mColumns; // number of columns the months will be dispersed into
	private int mWidth; // dimensions of the container housing the entire year
						// view
	private int mHeight;
	...
	public YearViewAdapter(Context context, int year, int columns) {
		...
		mCurrentMonthBgColor = mContext.getResources().getColor(R.color.year_view_current_month_bg_color);
	}

	/**
	 * Sets the container bounds that the entire year view can occupy. Drawing
	 * parameters are calculated based off these bounds
	 *
	 * @param width
	 *            max width of the parent container
	 * @param height
	 *            max height of the parent container
	 */
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		AbsListView.LayoutParams params = new AbsListView.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
				mMonthHeight);
		...
		Time now = new Time();
		now.setToNow();
		JalaliDate nowDate = Jalali.gregorianToJalali(now);
		if (nowDate.month == month + 1 && nowDate.year == mJalaliYear) {
			mCurrentJalaliMonth = month;
		...
		configureParams.put(MonthView.VIEW_PARAMS_YEAR, mJalaliYear);
		...
		// The drawing parameters are calculated based on the actual container
		// dimensions and the
		...
		drawingParams.put(MonthView.CONFIG_MONTH_LABEL_TEXT_SIZE, (int) (mMonthHeight * MONTH_LABEL_FONT_SIZE));

		// text size is chosen so that there isn't an overlap in the horizontal
		// or vertical
		// directions
		double textSize = Math.min(mMonthHeight * MONTH_DAY_TEXT_SIZE, mMonthWidth * MONTH_DAY_TEXT_SIZE);
		// add text size for month days
		drawingParams.put(MonthView.CONFIG_MONTH_DAY_TEXT_SIZE, (int) textSize);

		// header label flag
		drawingParams.put(MonthView.CONFIG_MONTH_HEADER_LABEL_FLAGS, DateUtils.FORMAT_SHOW_DATE
				| DateUtils.FORMAT_NO_YEAR | DateUtils.FORMAT_NO_MONTH_DAY);

		// add horizontal padding
		drawingParams.put(MonthView.CONFIG_MONTH_ROW_HEIGHT, (int) (mMonthHeight * ROW_HEIGHT));
		...
		linearLayout.setRotationY(180f);
		return linearLayout;
	}
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			mClickY = event.getY();
			mClickedView = v;
			// delay click acknowledgement to avoid view highlighting during
			// scroll
			mClickedView.postDelayed(mHighlightView, ViewConfiguration.getTapTimeout());
			break;
		case MotionEvent.ACTION_UP:
			if (v == mClickedView) {
				// avoid abrupt transitions in the UI by ensuring that the click
				// animation
				// persists for the minimum defined duration
				long tapDuration = System.currentTimeMillis() - mClickTimestamp;
				long delay = (tapDuration > mClickAnimDuration) ? 0 : mClickAnimDuration - tapDuration;
				v.postDelayed(mPerformClick, delay);
			}
			break;
		case MotionEvent.ACTION_MOVE:
			// clear view highlighting when the gesture becomes a scroll
			if ((Math.abs(event.getX() - mClickX) > mTouchSlop) || (Math.abs(event.getY() - mClickY) > mTouchSlop)) {
				clearClickedView(v);
			}
			break;
	...
	private void clearClickedView(View v) {
		...
		synchronized (v) {
			...
			if (v.getId() == mCurrentJalaliMonth) {
	...
	/**
	 * A runnable that completes the action mandated by a touch event. Used to
	 * delay the action consequence whilst the click animation is being
	 * performed.
	 */
	private final Runnable mPerformClick = new Runnable() {
		@Override
		public void run() {
			JalaliDate jDate = new JalaliDate(mJalaliYear, mClickedView.getId() + 1, 1);
			Time time = Jalali.jalaliToGregorianTime(jDate);
			// set(day, month, year)
			mController.sendEvent(mContext, EventType.GO_TO, time, time, -1, CalendarController.ViewType.MONTH,
					CalendarController.EXTRA_GOTO_BACK_TO_PREVIOUS, null, null);
		}
	};

	/**
	 * A greedy LinearLayout that always intercepts the touch events from its
	 * children.
	 */
	private static class GreedyLinearLayout extends LinearLayout {
		 ...
		 * @param ev
		 *            touch event
		 * @return always true indicating that the touch events won't be
	...
	/**
	 * Customized MonthView dictating how the days in a month are drawn onto the
	 * canvas
	 */
	public static class MonthViewImpl extends MonthView {
		 ...
		 */
		@Override
		public void drawMonthDay(Canvas canvas, int year, int month, int day, int x, int y, int startX, int stopX,
				int startY, int stopY) {

			if (mHasToday && mToday == day) {
				int radius = mRowHeight / 2;
				canvas.drawCircle(x, y - (mMiniDayNumberTextSize / 3), radius, mSelectedCirclePaint);
				mMonthNumPaint.setColor(mTodayNumberColor);
			} else {
				mMonthNumPaint.setColor(mDayTextColor);
			}
			canvas.drawText(Jalali.persianDigits(String.format("%d", day)), x, y, mMonthNumPaint);
		}
	}
}
