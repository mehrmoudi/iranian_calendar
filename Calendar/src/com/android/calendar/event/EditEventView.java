...
public class EditEventView implements View.OnClickListener, DialogInterface.OnCancelListener,
		DialogInterface.OnClickListener, OnItemSelectedListener, RecurrencePickerDialog.OnRecurrenceSetListener,
		...
		public void onClick(View v) {
			...
			if (v == mStartTimeButton) {
				...
				if (mStartTimePickerDialog == null) {
					mStartTimePickerDialog = TimePickerDialog.newInstance(new TimeListener(v), mTime.hour,
				...
				if (mEndTimePickerDialog == null) {
					mEndTimePickerDialog = TimePickerDialog.newInstance(new TimeListener(v), mTime.hour, mTime.minute,
	...
	private void populateTimezone(long eventStartTime) {
		...
		CharSequence displayName = mTzPickerUtils.getGmtDisplayName(mActivity, mTimezone, eventStartTime, true);
	...
	private class DateClickListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			...
			mDatePickerDialog = DatePickerDialog.newInstance(listener, mTime.year, mTime.month, mTime.monthDay);
			...
			mDatePickerDialog.show(mActivity.getSupportFragmentManager(), FRAG_TAG_DATE_PICKER);
		}
	}
	...
	public EditEventView(FragmentActivity activity, View view, EditDoneRunnable done, boolean timeSelectedWasStartTime,
	...
	private void prepareAvailability() {
		...
		mAvailabilityAdapter = new ArrayAdapter<String>(mActivity, android.R.layout.simple_spinner_item,
				mAvailabilityLabels);
	...
	public void updateHeadlineColor(CalendarEventModel model, int displayColor) {
		if (model.mUri != null) {
			if (mIsMultipane) {
				mView.findViewById(R.id.calendar_textview_with_colorpicker).setBackgroundColor(displayColor);
			} else {
		...
		if (mode == Utils.MODIFY_UNINITIALIZED || !EditEventHelper.canModifyEvent(mModel)) {
			...
			for (View v : mEditViewList) {
				...
				if (v.getTag() != null) {
					...
					v.setPadding(mOriginalPadding[0], mOriginalPadding[1], mOriginalPadding[2], mOriginalPadding[3]);
				}
			}
			if (mModel.mUri == null) {
		...
		while (calendarsCursor.moveToNext()) {
			...
			if (defaultCalendar == null) {
				...
				if (calendarOwner != null && calendarOwner.equals(calendarsCursor.getString(accountNameIndex))
						&& !CalendarContract.ACCOUNT_TYPE_LOCAL.equals(calendarsCursor.getString(accountTypeIndex))) {
					return position;
				}
	...
	private void addReminder() {
		...
		if (mDefaultReminderMinutes == GeneralPreferences.NO_REMINDER) {
			EventViewUtils
					.addReminder(mActivity, mScrollView, this, mReminderItems, mReminderMinuteValues,
			...
			EventViewUtils.addReminder(mActivity, mScrollView, this, mReminderItems, mReminderMinuteValues,
					mReminderMinuteLabels, mReminderMethodValues, mReminderMethodLabels,
					ReminderEntry.valueOf(mDefaultReminderMinutes), mModel.mCalendarMaxReminders, null);
		}
		updateRemindersVisibility(mReminderItems.size());
		EventViewUtils.updateAddReminderButton(mView, mReminderItems, mModel.mCalendarMaxReminders);
	}
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
		...
		LinearLayout reminderLayout = (LinearLayout) mScrollView.findViewById(R.id.reminder_items_container);
	...
	private void updateHomeTime() {
		...
		if (!mAllDayCheckBox.isChecked() && !TextUtils.equals(tz, mTimezone)
			...
			String tzDisplay = TimeZone.getTimeZone(tz).getDisplayName(isDSTStart, TimeZone.SHORT, Locale.getDefault());
			...
			mSB.setLength(0);
			mStartDateHome.setText(DateUtils.formatDateRange(mActivity, mF, millisStart, millisStart, flags, tz)
					.toString());
			...
			flags = DateUtils.FORMAT_ABBREV_ALL | DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_SHOW_YEAR
					| DateUtils.FORMAT_SHOW_WEEKDAY;
			mSB.setLength(0);
			mEndDateHome.setText(DateUtils.formatDateRange(mActivity, mF, millisEnd, millisEnd, flags, tz).toString());
