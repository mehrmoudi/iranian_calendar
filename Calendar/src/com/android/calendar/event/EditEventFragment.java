...
public class EditEventFragment extends Fragment implements EventHandler, OnColorSelectedListener {
	...
	private static final int TOKEN_ALL = TOKEN_EVENT | TOKEN_ATTENDEES | TOKEN_REMINDERS | TOKEN_CALENDARS
			| TOKEN_COLORS;
		...
		protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
			...
			case TOKEN_EVENT:
				...
				if (mModel.mHasAttendeeData && eventId != -1) {
					...
					mHandler.startQuery(TOKEN_ATTENDEES, null, attUri, EditEventHelper.ATTENDEES_PROJECTION,
							EditEventHelper.ATTENDEES_WHERE /* selection */, whereArgs /*
																						 * selection
																						 * args
																						 */, null /*
																								 * sort
				...
				if (mModel.mHasAlarm && mReminders == null) {
					...
					mHandler.startQuery(TOKEN_REMINDERS, null, rUri, EditEventHelper.REMINDERS_PROJECTION,
							EditEventHelper.REMINDERS_WHERE /* selection */, remArgs /*
																					 * selection
																					 * args
																					 */, null /*
																							 * sort
					...
					mModel.mReminders = (ArrayList<ReminderEntry>) mReminders.clone();
					setModelIfDone(TOKEN_REMINDERS);
				}

				// TOKEN_CALENDARS
				String[] selArgs = { Long.toString(mModel.mCalendarId) };
				mHandler.startQuery(TOKEN_CALENDARS, null, Calendars.CONTENT_URI, EditEventHelper.CALENDARS_PROJECTION,
						EditEventHelper.CALENDARS_WHERE, selArgs /*
																 * selection
																 * args
																 */, null /*
																		 * sort
				...
				// TOKEN_COLORS
				mHandler.startQuery(TOKEN_COLORS, null, Uri.parse("content://com.android.calendar/colors"),
						EditEventHelper.COLORS_PROJECTION, "color_type" + "=" + 1, null, null);
			...
			case TOKEN_ATTENDEES:
				try {
					while (cursor.moveToNext()) {
						...
						int relationship = cursor.getInt(EditEventHelper.ATTENDEES_INDEX_RELATIONSHIP);
						if (relationship == Attendees.RELATIONSHIP_ORGANIZER) {
							if (email != null) {
								mModel.mOrganizer = email;
								mModel.mIsOrganizer = mModel.mOwnerAccount.equalsIgnoreCase(email);
								mOriginalModel.mOrganizer = email;
								mOriginalModel.mIsOrganizer = mOriginalModel.mOwnerAccount.equalsIgnoreCase(email);
							}

							if (TextUtils.isEmpty(name)) {
								mModel.mOrganizerDisplayName = mModel.mOrganizer;
								mOriginalModel.mOrganizerDisplayName = mOriginalModel.mOrganizer;
							} else {
						...
						if (email != null) {
							if (mModel.mOwnerAccount != null && mModel.mOwnerAccount.equalsIgnoreCase(email)) {
								int attendeeId = cursor.getInt(EditEventHelper.ATTENDEES_INDEX_ID);
			...
			case TOKEN_CALENDARS:
				try {
					if (mModel.mId == -1) {
						...
						mView.setCalendarsCursor(matrixCursor, isAdded() && isResumed(), mCalendarId);
					} else {
			...
			case TOKEN_COLORS:
				if (cursor.moveToFirst()) {
					...
					do {
						...
						String accountName = cursor.getString(EditEventHelper.COLORS_INDEX_ACCOUNT_NAME);
						String accountType = cursor.getString(EditEventHelper.COLORS_INDEX_ACCOUNT_TYPE);
				...
				// If the account name/type is null, the calendar event colors
				// cannot be
				// determined, so take the default/savedInstanceState value.
				if (mModel.mCalendarAccountName == null || mModel.mCalendarAccountType == null) {
	...
	private View.OnClickListener mOnColorPickerClicked = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (mColorPickerDialog == null) {
				mColorPickerDialog = EventColorPickerDialog.newInstance(colors, mModel.getEventColor(),
	...
	public EditEventFragment(EventInfo event, ArrayList<ReminderEntry> reminders, boolean eventColorInitialized,
			int eventColor, boolean readOnly, Intent intent) {
		mEvent = event;
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		mColorPickerDialog = (EventColorPickerDialog) getActivity().getSupportFragmentManager().findFragmentByTag(
				COLOR_PICKER_DIALOG_TAG);
		if (mColorPickerDialog != null) {
	...
	private void startQuery() {
		...
		if (!newEvent) {
			...
			mHandler.startQuery(TOKEN_EVENT, null, mUri, EditEventHelper.EVENT_PROJECTION, null /* selection */,
			...
			// Start a query in the background to read the list of calendars and
			// colors
			mHandler.startQuery(TOKEN_CALENDARS, null, Calendars.CONTENT_URI, EditEventHelper.CALENDARS_PROJECTION,
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		mView = new EditEventView(mActivity, view, mOnDone, mTimeSelectedWasStartTime, mDateSelectedWasStartDate);
		startQuery();

		if (mUseCustomActionBar) {
			View actionBarButtons = inflater.inflate(R.layout.edit_event_custom_actionbar, new LinearLayout(mActivity),
					false);
	...
	/**
	 * Handles menu item selections, whether they come from our custom action
	 * bar buttons or from the standard menu items. Depends on the menu item ids
	 * matching the custom action bar button ids.
	 *
	 * @param itemId
	 *            the button or menu item id
	...
	private boolean onActionBarItemSelected(int itemId) {
		if (itemId == R.id.action_done) {
			...
			} else if (EditEventHelper.canAddReminders(mModel) && mModel.mId != -1 && mOriginalModel != null
	...
	private void saveReminders() {
		...
		boolean changed = EditEventHelper.saveReminders(ops, mModel.mId, mModel.mReminders, mOriginalModel.mReminders,
				false /* no force save */);
	...
	protected void displayEditWhichDialog() {
		if (mModification == Utils.MODIFY_UNINITIALIZED) {
			...
			// Do one more check to make sure this remains at the end of the
			// list
			if (!isFirstEventInSeries) {
	...
	class Done implements EditEventHelper.EditDoneRunnable {
		...
		public void run() {
			...
			if ((mCode & Utils.DONE_SAVE) != 0 && mModel != null
					...
					&& mView.prepareForSave() && !isEmptyNewEvent() && mModel.normalizeReminders()
			...
			if ((mCode & Utils.DONE_DELETE) != 0 && mOriginalModel != null
				...
				DeleteEventHelper deleteHelper = new DeleteEventHelper(mActivity, mActivity, !mIsReadOnly /* exitWhenDone */);
			...
			// Hide a software keyboard so that user won't see it even after
			// this Fragment's
			// disappearing.
	...
	public void onPause() {
		...
		if (mSaveOnDetach && act != null && !mIsReadOnly && !act.isChangingConfigurations() && mView.prepareForSave()) {
	...
	public void handleEvent(EventInfo event) {
		...
		if ((false && event.eventType == EventType.USER_HOME) || (event.eventType == EventType.GO_TO && mSaveOnDetach)) {
			if (mView != null && mView.prepareForSave()) {
				mOnDone.setDoneCode(Utils.DONE_SAVE);
				mOnDone.run();
			}
