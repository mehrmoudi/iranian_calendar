...
public class Utils {
	...
	public static final int JALALI_YEAR_MIN = 1348;
	public static final int JALALI_YEAR_MAX = 1415;
	...
	/**
	 * A coordinate must be of the following form for Google Maps to correctly
	 * use it: Latitude, Longitude
	 *
	 * This may be in decimal form: Latitude: {-90 to 90} Longitude: {-180 to
	 * 180}
	 *
	 * Or, in degrees, minutes, and seconds: Latitude: {-90 to 90}�� {0 to 59}'
	 * {0 to 59}" Latitude: {-180 to 180}�� {0 to 59}' {0 to 59}" + or - degrees
	 * may also be represented with N or n, S or s for latitude, and with E or
	 * e, W or w for longitude, where the direction may either precede or follow
	 * the value.
	 *
	 * Some examples of coordinates that will be accepted by the regex:
	 * 37.422081��, -122.084576�� 37.422081,-122.084576 +37��25'19.49",
	 * -122��5'4.47" 37��25'19.49"N, 122��5'4.47"W N 37�� 25'
	 * 19.49",  W 122�� 5' 4.47"
	 **/
	private static final String COORD_DEGREES_LATITUDE = "([-+NnSs]" + "(\\s)*)?" + "[1-9]?[0-9](\u00B0)" + "(\\s)*"
			+ "([1-5]?[0-9]\')?" + "(\\s)*" + "([1-5]?[0-9]" + "(\\.[0-9]+)?\")?" + "((\\s)*" + "[NnSs])?";
	private static final String COORD_DEGREES_LONGITUDE = "([-+EeWw]" + "(\\s)*)?" + "(1)?[0-9]?[0-9](\u00B0)"
			+ "(\\s)*" + "([1-5]?[0-9]\')?" + "(\\s)*" + "([1-5]?[0-9]" + "(\\.[0-9]+)?\")?" + "((\\s)*" + "[EeWw])?";
	private static final String COORD_DEGREES_PATTERN = COORD_DEGREES_LATITUDE + "(\\s)*" + "," + "(\\s)*"
	...
	private static final String COORD_DECIMAL_PATTERN = COORD_DECIMAL_LATITUDE + "(\\s)*" + "," + "(\\s)*"
			+ COORD_DECIMAL_LONGITUDE;
	private static final Pattern COORD_PATTERN = Pattern.compile(COORD_DEGREES_PATTERN + "|" + COORD_DECIMAL_PATTERN);
	...
	public static boolean getDefaultVibrate(Context context, SharedPreferences prefs) {
		...
		if (prefs.contains(KEY_ALERTS_VIBRATE_WHEN)) {
			...
			vibrate = vibrateWhen != null
					&& vibrateWhen.equals(context.getString(R.string.prefDefault_alerts_vibrate_true));
			prefs.edit().remove(KEY_ALERTS_VIBRATE_WHEN).commit();
			Log.d(TAG, "Migrating KEY_ALERTS_VIBRATE_WHEN(" + vibrateWhen + ") to KEY_ALERTS_VIBRATE = " + vibrate);
		} else {
	...
	public static void removeSharedPreference(Context context, String key) {
		SharedPreferences prefs = context.getSharedPreferences(GeneralPreferences.SHARED_PREFS_NAME,
	...
	// backed-up shared_pref file (SHARED_PREFS_NAME) is used only to control
	// the
	...
	public static String getRingTonePreference(Context context) {
		SharedPreferences prefs = context.getSharedPreferences(GeneralPreferences.SHARED_PREFS_NAME_NO_BACKUP,
				Context.MODE_PRIVATE);
...
//		}
	...
	/**
	 * Converts the day of the week from android.text.format.Time to
	 * java.util.Calendar
	 */
	...
	public static long getDefaultSnoozeDelayMs(Context context) {
		...
		final long intValue = value != null ? Long.valueOf(value) : GeneralPreferences.SNOOZE_DELAY_DEFAULT_TIME;
	...
	public static boolean isFriday(int column, int firstDayOfWeek) {
		return (firstDayOfWeek == Time.SUNDAY && column == 5) || (firstDayOfWeek == Time.MONDAY && column == 4)
	...
	/**
	 * For devices with Jellybean or later, darkens the given color to ensure
	 * that white text is clearly visible on top of it. For devices prior to
	 * Jellybean, does nothing, as the sync adapter handles the color change.
	 *
	 * @param color
	 */
	...
	public static void trySyncAndDisableUpgradeReceiver(Context context) {
		...
		if (pm.getComponentEnabledSetting(upgradeComponent) == PackageManager.COMPONENT_ENABLED_STATE_DISABLED) {
			// The upgrade receiver has been disabled, which means this code has
			// been run before,
			// so no need to sync.
			return;
		}
		...
		ContentResolver.requestSync(null /* no account */, Calendars.CONTENT_URI.getAuthority(), extras);

		// Now unregister the receiver so that we won't continue to sync every
		// time.
		pm.setComponentEnabledSetting(upgradeComponent, PackageManager.COMPONENT_ENABLED_STATE_DISABLED,
	...
	public static HashMap<Integer, DNAStrand> createDNAStrands(int firstJulianDay, ArrayList<Event> events, int top,
		...
		if (events == null || events.isEmpty() || dayXs == null || dayXs.length < 1 || bottom - top < 8
				|| minPixels < 0) {
//			Log.e(TAG, "Bad values for createDNAStrands! events:" + events + " dayXs:" + Arrays.toString(dayXs)
//					+ " bot-top:" + (bottom - top) + " minPixels:" + minPixels);
			return null;
		}
	...
	public static int getWeekNumberFromTime(long millisSinceEpoch, Context context) {
		...
		return Jalali.gregorianToJalali(weekTime).getWeekNumber();
	}
	...
	public static Intent createEmailAttendeesIntent(Resources resources, String eventTitle, String body,
		...
		// Workaround a Email bug that overwrites the body with this intent
		// extra. If not
	...
	public static void setTodayIcon(LayerDrawable icon, Context c, String timezone) {
		...
		now.setToNow();
		now.normalize(false);
		today.setDayOfMonth(Jalali.gregorianToJalali(now).day);
		icon.mutate();
	 ...
	 *
	 * If a Calendar is added on the web (and it is selected and not hidden)
	 * then it will be added to the list of calendars on the phone (when this
	 * finishes). When a new calendar from the web is added to the phone, then
	 * the events for that calendar are also downloaded from the web.
	 *
	 * This sync is done automatically in the background when the
	 * SelectCalendars activity and fragment are started.
	 *
	 * @param account
	 *            - The account to sync. May be null to sync all accounts.
	 */
	...
	/**
	 * Replaces stretches of text that look like addresses and phone numbers
	 * with clickable links. If lastDitchGeo is true, then if no links are found
	 * in the textview, the entire string will be converted to a single geo
	 * link. Any spans that may have previously been in the text will be cleared
	 * out.
	 * <p>
	 ...
	 * @param lastDitchGeo
	 *            - If no links are found, turn the entire string into one geo
	 *            link.
	...
	public static Spannable extendedLinkify(String text, boolean lastDitchGeo) {
		// We use a copy of the string argument so it's available for later if
		// necessary.
		Spannable spanText = SpannableString.valueOf(text);

		/*
		 * If the text includes a street address like
		 * "1600 Amphitheater Parkway, 94043", the current Linkify code will
		 * identify "94043" as a phone number and invite you to dial it (and not
		 * provide a map link for the address). For outside US, use Linkify
		 * result iff it spans the entire text. Otherwise send the user to maps.
		 */
		...
		if (!defaultPhoneRegion.equals("US")) {
			...
			// Otherwise, to be cautious and to try to prevent false positives,
			// reset the spannable.
		...
		/*
		 * For within US, we want to have better recognition of phone numbers
		 * without losing any of the existing annotations. Ideally this would be
		 * addressed by improving Linkify. For now we manage it as a second pass
		 * over the text.
		 * 
		 * URIs and e-mail addresses are pretty easy to pick out of text. Phone
		 * numbers are a bit tricky because they have radically different
		 * formats in different countries, in terms of both the digits and the
		 * way in which they are commonly written or presented (e.g. the
		 * punctuation and spaces in "(650) 555-1212"). The expected format of a
		 * street address is defined in WebView.findAddress(). It's pretty
		 * narrowly defined, so it won't often match.
		 * 
		 * The RFC 3966 specification defines the format of a "tel:" URI.
		 * 
		 * Start by letting Linkify find anything that isn't a phone number. We
		 * have to let it run first because every invocation removes all
		 * previous URLSpan annotations.
		 * 
		...
		/*
		 * Get a list of any spans created by Linkify, for the coordinate
		 * overlapping span check.
		 */
		URLSpan[] existingSpans = spanText.getSpans(0, spanText.length(), URLSpan.class);

		/*
		 * Check for coordinates. This must be done before phone numbers because
		 * longitude may look like a phone number.
		 */
		...
		/*
		 * Update the list of existing spans, for the phone number overlapping
		 * span check.
		 */
		 ...
		 * 
		 * Some URIs contain strings of digits that look like phone numbers. If
		 * both the URI scanner and the phone number scanner find them, we want
		 * the URI link to win. Since the URI scanner runs first, we just need
		 * to avoid creating overlapping spans.
		 */
		...
		for (int match = 0; match < phoneSequences.length / 2; match++) {
			...
			/*
			 * The Linkify code takes the matching span and strips out
			 * everything that isn't a digit or '+' sign. We do the same here.
			 * Extension numbers will get appended without a separator, but the
			 * dialer wasn't doing anything useful with ";ext=" anyway.
			 */
		...
		/*
		 * If lastDitchGeo, and no other links have been found, set the entire
	...
	/**
	 * Finds North American Numbering Plan (NANP) phone numbers in the input
	 * text.
	 *
	 * @param text
	 *            The text to scan.
	 * @return A list of [start, end) pairs indicating the positions of phone
	...
	static int[] findNanpPhoneNumbers(CharSequence text) {
		...
		/*
		 * We can't just strip the whitespace out and crunch it down, because
		 * the whitespace is significant. March through, trying to figure out
		 * where numbers start and end.
		 */
	...
	/**
	 * Checks to see if there is a valid phone number in the input, starting at
	 * the specified offset. If so, the index of the last character + 1 is
	 * returned. The input is assumed to begin with a non-whitespace character.
	 *
	 * @return Exclusive end position, or -1 if not a match.
	 */
	private static int findNanpMatchEnd(CharSequence text, int startPos) {
		/*
		 * A few interesting cases: 94043 # too short, ignore 123456789012 # too
		 * long, ignore +1 (650) 555-1212 # 11 digits, spaces (650) 555 5555 #
		 * Second space, only when first is present. (650) 555-1212, (650)
		 * 555-1213 # two numbers, return first 1-650-555-1212 # 11 digits with
		 * leading '1' *#650.555.1212#*! # 10 digits, include #*, ignore
		 * trailing '!' 555.1212 # 7 digits
		 * 
		 * For the most part we want to break on whitespace, but it's common to
	...
	/**
	 * @param bundle
	 *            The incoming bundle that contains the reminder info.
	...
	public static ArrayList<ReminderEntry> readRemindersFromBundle(Bundle bundle) {
		...
		if (numReminders == reminderMethods.size()) {
			...
			Log.d(TAG, String.format("Error resolving reminders."
					+ " Found %d reminderMinutes, but %d reminderMethods.", numReminders, reminderMethods.size()));
		}

		return reminders;
	}

	public static boolean showGregorian() {
		return GeneralPreferences.getSharedPreferences(CalendarApplication.getContext()).getBoolean(
				"preferences_show_gregorian_date", true);
	}

	public static boolean showHijri() {
		return GeneralPreferences.getSharedPreferences(CalendarApplication.getContext()).getBoolean(
				"preferences_show_hijri_date", true);
	}
