...
public class CalendarController {
	...
	public static class EventInfo {
		...
		/**
		 * For EventType.VIEW_EVENT: It is the default attendee response and an
		 * all day event indicator. Set to Attendees.ATTENDEE_STATUS_NONE,
		 * Attendees.ATTENDEE_STATUS_ACCEPTED,
		 * Attendees.ATTENDEE_STATUS_DECLINED, or
		 * Attendees.ATTENDEE_STATUS_TENTATIVE. To signal the event is an
		 * all-day event, "or" ALL_DAY_MASK with the response. Alternatively,
		 * use buildViewExtraLong(), getResponse(), and isAllDay().
		 * <p>
		 * For EventType.CREATE_EVENT: Set to {@link #EXTRA_CREATE_ALL_DAY} for
		 * creating an all-day event.
		 * <p>
		 * For EventType.GO_TO: Set to {@link #EXTRA_GOTO_TIME} to go to the
		 * specified date/time. Set to {@link #EXTRA_GOTO_DATE} to consider the
		 * date but ignore the time. Set to {@link #EXTRA_GOTO_BACK_TO_PREVIOUS}
		 * if back should bring back previous view. Set to
		 * {@link #EXTRA_GOTO_TODAY} if this is a user request to go to the
		 * current time.
		 * <p>
		 * For EventType.UPDATE_TITLE: Set formatting flags for
	...
	/**
	 * Pass to the ExtraLong parameter for EventType.CREATE_EVENT to create an
	 * all-day event
	 */
	 ...
	 *
	 * @param context
	 *            The activity used to create the controller
	 */
	...
	private CalendarController(Context context) {
		...
		mUpdateTimezone.run();
		mTime.setToNow();
		mDetailViewType = Utils.getSharedPreference(mContext, GeneralPreferences.KEY_DETAILED_VIEW,
				GeneralPreferences.DEFAULT_DETAILED_VIEW);
	}

	public void sendEventRelatedEvent(Object sender, long eventType, long eventId, long startMillis, long endMillis,
			int x, int y, long selectedMillis) {
		// TODO: pass the real allDay status or at least a status that says we
		// don't know the
		// status and have the receiver query the data.
		// The current use of this method for VIEW_EVENT is by the day view to
		// show an EventInfo
		...
		sendEventRelatedEventWithExtra(sender, eventType, eventId, startMillis, endMillis, x, y,
				EventInfo.buildViewExtraLong(Attendees.ATTENDEE_STATUS_NONE, false), selectedMillis);
	}
	 ...
	 * @param y
	 *            y coordinate in the activity space
	 * @param extraLong
	 *            default response value for the "simple event view" and all day
	 *            indication. Use Attendees.ATTENDEE_STATUS_NONE for no
	 *            response.
	 * @param selectedMillis
	 *            The time to specify as selected
	 */
	public void sendEventRelatedEventWithExtra(Object sender, long eventType, long eventId, long startMillis,
			long endMillis, int x, int y, long extraLong, long selectedMillis) {
		sendEventRelatedEventWithExtraWithTitleWithCalendarId(sender, eventType, eventId, startMillis, endMillis, x, y,
				extraLong, selectedMillis, null, -1);
	}
	 ...
	 * @param calendarId
	 *            The id of the calendar which the event belongs to
	 */
	public void sendEventRelatedEventWithExtraWithTitleWithCalendarId(Object sender, long eventType, long eventId,
			long startMillis, long endMillis, int x, int y, long extraLong, long selectedMillis, String title,
			long calendarId) {
	...
	/**
	 * sendEvent() variant with extraLong, search query, and search component
	 * name.
	 */
	public void sendEvent(Object sender, long eventType, Time start, Time end, long eventId, int viewType,
	...
	public void sendEvent(Object sender, long eventType, Time start, Time end, Time selected, long eventId,
	...
	public void sendEvent(Object sender, final EventInfo event) {
		...
		if (event.selectedTime != null && event.selectedTime.toMillis(false) != 0) {
			...
			if (startMillis != 0) {
				// selectedTime is not set so set mTime to startTime iff it is
				// not
	...
	/**
	 * Adds or updates an event handler. This uses a LinkedHashMap so that we
	 * can replace fragments based on the view id they are being expanded into.
	 *
	 * @param key
	 *            The view id or placeholder for this handler
	 * @param eventHandler
	 *            Typically a fragment or activity in the calendar app
	 */
	...
	public void deregisterEventHandler(Integer key) {
		synchronized (this) {
			if (mDispatchInProgressCounter > 0) {
				// To avoid ConcurrencyException, stash away the event handler
				// for now.
	...
	public Intent generateCreateEventIntent(long startMillis, long endMillis, boolean allDayEvent, String title,
			long calendarId) {
		Intent intent = new Intent(Intent.ACTION_VIEW);
	...
	private void launchDeleteEventAndFinish(Activity parentActivity, long eventId, long startMillis, long endMillis,
			int deleteWhich) {
		DeleteEventHelper deleteEventHelper = new DeleteEventHelper(mContext, parentActivity, parentActivity != null /*
																													 * exit
																													 * when
																													 * done
																													 */);
