...
import com.android.calendar.jalali.DateUtils;
...
public class DayView extends View implements View.OnCreateContextMenuListener,
	...
	private static float mScale = 0; // Used for supporting different screen
										// densities
	private static final long INVALID_EVENT_ID = -1; // This is used for
														// remembering a null
														// event
	 ...
	 *
	 * This is used to prevent popping up multiple quick views for the same
	 * event, especially during calendar syncs. This becomes valid when an event
	 * is selected, either by default on starting calendar or by scrolling to an
	...
	private static final String[] CALENDARS_PROJECTION = new String[] { Calendars._ID, // 0
		...
		public void run() {
			if (mClickedEvent != null) {
				mController.sendEventRelatedEvent(this, EventType.VIEW_EVENT, mClickedEvent.id,
						mClickedEvent.startMillis, mClickedEvent.endMillis, DayView.this.getWidth() / 2,
		...
		public void onAnimationEnd(Animator animation) {
			synchronized (this) {
				...
				if (mFadingIn) {
					...
					mTodayAnimator = ObjectAnimator.ofInt(DayView.this, "animateTodayAlpha", 255, 0);
	...
	private static int mWeek_hollidayColor;
	...
	public DayView(Context context, CalendarController controller, ViewSwitcher viewSwitcher, EventLoader eventLoader,
			int numDays) {
		super(context);
		mContext = context;
		...
		if (mCellHeight == 0) {
			mCellHeight = Utils.getSharedPreference(mContext, GeneralPreferences.KEY_DEFAULT_CELL_HEIGHT,
					DEFAULT_CELL_HEIGHT);
		}
	...
	private void init(Context context) {
		...
		mWeek_hollidayColor = mResources.getColor(R.color.week_holliday);
		...
		for (int i = Calendar.SUNDAY; i <= Calendar.SATURDAY; i++) {
			...
			// e.g. Tue for Tuesday
			mDayStrs[index] = DateUtils.getDayOfWeekString(i, DateUtils.LENGTH_LONG).toUpperCase();
		...
		mAmString = mContext.getString(R.string.am);
		mPmString = mContext.getString(R.string.pm);
		...
		mPopupView.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
	...
	private void initAccessibilityVariables() {
		mAccessibilityMgr = (AccessibilityManager) mContext.getSystemService(Service.ACCESSIBILITY_SERVICE);
	...
	public void setSelected(Time time, boolean ignoreTime, boolean animateToday) {
		...
		if (!ignoreTime && mGridAreaHeight != -1) {
			...
			if (mBaseDate.hour < mFirstHour) {
				...
				lastHour = (mGridAreaHeight - mFirstHourOffset) / (mCellHeight + HOUR_GAP) + mFirstHour;
				...
				if (mBaseDate.hour >= lastHour) {
					...
					// region)
					gotoY = (int) ((mBaseDate.hour + 1 + mBaseDate.minute / 60.0f) * (mCellHeight + HOUR_GAP) - mGridAreaHeight);
				}
			}

			if (DEBUG) {
				Log.e(TAG, "Go " + gotoY + " 1st " + mFirstHour + ":" + mFirstHourOffset + "CH "
						+ (mCellHeight + HOUR_GAP) + " lh " + lastHour + " gh " + mGridAreaHeight + " ymax "
						+ mMaxViewStartY);
			}
		...
		if (animateToday) {
			...
			synchronized (mTodayAnimatorListener) {
				...
				mTodayAnimator = ObjectAnimator.ofInt(this, "animateTodayAlpha", mAnimateTodayAlpha, 255);
				mAnimateToday = true;
	...
	public void updateTitle() {
		...
		mController.sendEvent(this, EventType.UPDATE_TITLE, start, end, null, -1, ViewType.CURRENT, formatFlags, null,
				null);
	}
	 ...
	 *
	 * @param trackBallSelection
	 *            true if the selection was made using the trackball.
	 */
		...
		if (mNumDays > 1) {
			...
			if (trackBallSelection) {
				if (selectedEvent == null) {
					...
					mController.sendEventRelatedEventWithExtra(this, EventType.CREATE_EVENT, -1, startMillis,
							endMillis, -1, -1, extraLong, -1);
				} else {
					...
					mController.sendEventRelatedEvent(this, EventType.VIEW_EVENT, selectedEvent.id,
							selectedEvent.startMillis, selectedEvent.endMillis, 0, 0, getSelectedTimeInMillis());
				}
			} else {
			...
			if (selectedEvent == null) {
				...
				mController.sendEventRelatedEventWithExtra(this, EventType.CREATE_EVENT, -1, startMillis, endMillis,
						-1, -1, extraLong, -1);
			} else {
				if (mIsAccessibilityEnabled) {
					mAccessibilityMgr.interrupt();
					...
					if (mSelectedEventForAccessibility == null) {
						...
						if (numEvents > 1) {
							...
							b.append(mFormatter.format(mEventCountTemplate,
									mSelectedEvents.indexOf(mSelectedEventForAccessibility) + 1, numEvents));
							b.append(" ");
						}
	...
	private class GotoBroadcaster implements Animation.AnimationListener {
		...
		public void onAnimationEnd(Animation animation) {
			...
			if (mCounter == sCounter) {
				mController.sendEvent(this, EventType.GO_TO, mStart, mEnd, null, -1, ViewType.CURRENT,
	...
	private void adjustHourSelection() {
		...
		if (mSelectionHour < mFirstHour + 1) {
			...
			if (daynum < mEarliestStartHour.length && daynum >= 0 && mMaxAlldayEvents > 0
					&& mEarliestStartHour[daynum] > mSelectionHour && mFirstHour > 0 && mFirstHour < 8) {
				mPrevSelectedEvent = null;
				mSelectionAllday = true;
				setSelectedHour(mFirstHour + 1);
				return;
	...
	private void drawScrollLine(Rect r, Canvas canvas, Paint p) {
		final int right = computeDayRightPosition(0) + mHoursWidth;
	...
	// return day * effectiveWidth / mNumDays;
	// }

	// Computes the x position for the right side of the given day (base 0)
	private int computeDayRightPosition(int day) {
		day = mNumDays - day;
		int effectiveWidth = mViewWidth - mHoursWidth;
		return day * effectiveWidth / mNumDays;
	}
	...
	private void drawAllDayHighlights(Rect r, Canvas canvas, Paint p) {
		if (mFutureBgColor != 0) {
			...
			if (startIndex >= 0) {
				...
				r.top = 0;
				r.bottom = mFirstCell - 1;
				r.right = computeDayRightPosition(startIndex) + 1;
				r.left = computeDayRightPosition(mNumDays);
		...
		if (mSelectionAllday && mSelectionMode != SELECTION_HIDDEN) {
			...
			int daynum = mSelectionDay - mFirstJulianDay;
			mRect.right = computeDayRightPosition(daynum) + 1;
			mRect.left = computeDayRightPosition(daynum + 1);
	...
	private void drawAmPm(Canvas canvas, Paint p) {
		...
		canvas.drawText(text, mViewWidth - mHoursWidth + HOURS_LEFT_MARGIN, y, p);
	...
	private void drawCurrentTimeLine(Rect r, final int day, final int top, Canvas canvas, Paint p) {
		r.right = computeDayRightPosition(day) + CURRENT_TIME_LINE_SIDE_BUFFER + 1;
		r.left = computeDayRightPosition(day + 1) - CURRENT_TIME_LINE_SIDE_BUFFER + 1;
	...
	private void drawSelectedRect(Rect r, Canvas canvas, Paint p) {
		...
		if (mSelectionMode != SELECTION_HIDDEN && !mSelectionAllday) {
			...
			r.right = computeDayRightPosition(daynum) + 1;
			r.left = computeDayRightPosition(daynum + 1) - 1;
	...
	private void drawHours(Rect r, Canvas canvas, Paint p) {
		...
		for (int i = 0; i < 24; i++) {
			...
			canvas.drawText(time, mViewWidth - mHoursWidth + HOURS_LEFT_MARGIN, y, p);
	...
	private void drawDayHeader(String dayStr, int day, int cell, Canvas canvas, Paint p) {
		...
		if (mNumDays > 1) {
			...
			if (LunarUtils.showLunar(mContext)) {
				...
				String lunarInfo = LunarUtils.get(mContext, year, month, dateNum, LunarUtils.FORMAT_LUNAR_SHORT
						| LunarUtils.FORMAT_ONE_FESTIVAL, false, null);
	...
	private void drawGridBackground(Rect r, Canvas canvas, Paint p) {
		...
		final float stopX = computeDayRightPosition(0) + mHoursWidth;
		float y = 0;
	...
	private void drawBgColors(Rect r, Canvas canvas, Paint p) {
		...
		if (mNumDays == 1 && todayIndex == 0) {
			...
			int lineY = mCurrentTime.hour * (mCellHeight + HOUR_GAP) + ((mCurrentTime.minute * mCellHeight) / 60) + 1;
			...
			if (lineY < mViewStartY + mViewHeight) {
				lineY = Math.max(lineY, mViewStartY);
				r.right = computeDayRightPosition(todayIndex) + 1;
				r.left = computeDayRightPosition(todayIndex + 1);
				r.top = lineY;
				r.bottom = mViewStartY + mViewHeight;
				p.setColor(mFutureBgColor);
				canvas.drawRect(r, p);
			...
			if (todayIndex + 1 < mNumDays) {
				r.right = computeDayRightPosition(todayIndex + 1) + 1;
				r.left = computeDayRightPosition(mNumDays);
				r.top = mDestRect.top;
				r.bottom = mDestRect.bottom;
				p.setColor(mFutureBgColor);
			...
			// Future
			r.right = computeDayRightPosition(0) + 1;
			r.left = computeDayRightPosition(mNumDays);
			r.top = mDestRect.top;
			r.bottom = mDestRect.bottom;
			p.setColor(mFutureBgColor);
	...
	private Rect getCurrentSelectionPosition() {
		...
		int daynum = mSelectionDay - mFirstJulianDay;
		box.right = computeDayRightPosition(daynum) + 1;
		box.left = computeDayRightPosition(daynum + 1) - 1;
		return box;
	}
	...
	private void drawAllDayEvents(int firstDay, int numDays, Canvas canvas, Paint p) {
		...
		mLines[linesIndex++] = GRID_LINE_LEFT_MARGIN;
		mLines[linesIndex++] = startY;
		mLines[linesIndex++] = computeDayRightPosition(0);
		mLines[linesIndex++] = startY;
		...
		for (int i = 0; i < numEvents; i++) {
			...
			float height = mMaxAlldayEvents > mMaxUnexpandedAlldayEventCount ? mAnimateDayEventHeight : drawHeight
					/ numRectangles;
			...
			// event rectangle.
			event.right = computeDayRightPosition(startIndex);
			event.left = computeDayRightPosition(endIndex + 1) - DAY_GAP;
	...
	protected void drawMoreAlldayEvents(Canvas canvas, int remainingEvents, int day, Paint p) {
		int x = computeDayRightPosition(day) + EVENT_ALL_DAY_TEXT_LEFT_MARGIN;
		int y = (int) (mAlldayHeight - .5f * MIN_UNEXPANDED_ALLDAY_EVENT_HEIGHT - .5f * EVENT_SQUARE_WIDTH
				+ DAY_HEADER_HEIGHT + ALLDAY_TOP_MARGIN);
		Rect r = mRect;
		r.top = y;
		r.left = x;
	...
	private void drawEvents(int date, int dayIndex, int top, Canvas canvas, Paint p) {
		Paint eventTextPaint = mEventTextPaint;
		int right = computeDayRightPosition(dayIndex) - 1;
		int cellWidth = right - computeDayRightPosition(dayIndex + 1) + 1;
		...
		selectionArea.right = right;
		selectionArea.left = selectionArea.right - cellWidth;
		...
		for (int i = 0; i < numEvents; i++) {
			Event event = events.get(i);
			if (!geometry.computeEventRect(date, selectionArea.left, top, cellWidth, event)) {
				continue;
			}
			...
			drawEventText(layout, r, canvas, mViewStartY + 4, mViewStartY + mViewHeight - DAY_HEADER_HEIGHT
					- mAlldayHeight, false);
		}
		eventTextPaint.setAlpha(alpha);
	...
	private void computeNeighbors() {
		...
		if (mPrevSelectedEvent != null) {
			...
			if (prevTop >= mPrevBox.bottom || prevBottom <= mPrevBox.top || prevRight <= mPrevBox.left
					|| prevLeft >= mPrevBox.right) {
				mPrevSelectedEvent = null;
	...
	private void updateEventDetails() {
		if (mSelectedEvent == null || mSelectionMode == SELECTION_HIDDEN || mSelectionMode == SELECTION_LONGPRESS) {
			mPopup.dismiss();
			return;
		}
		...
		int flags;
		if (event.allDay) {
			flags = DateUtils.FORMAT_UTC | DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_SHOW_WEEKDAY
					| DateUtils.FORMAT_ABBREV_ALL;
		} else {
			flags = DateUtils.FORMAT_SHOW_TIME | DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_SHOW_WEEKDAY
					| DateUtils.FORMAT_ABBREV_ALL | DateUtils.FORMAT_CAP_NOON_MIDNIGHT;
		}
		if (DateFormat.is24HourFormat(mContext)) {
			flags |= DateUtils.FORMAT_24HOUR;
		}
		String timeRange = Utils.formatDateRange(mContext, event.startMillis, event.endMillis, flags);
	...
	private void doDown(MotionEvent ev) {
		...
		if (setSelectionFromPosition(x, y, false)) {
			...
			boolean pressedSelected = (mSelectionMode != SELECTION_HIDDEN) && oldSelectionDay == mSelectionDay
	...
	private void doExpandAllDayClick() {
		...
		if (mAnimateDayHeight == 0) {
			mAnimateDayHeight = mShowAllAllDayEvents ? mAlldayHeight - (int) MIN_UNEXPANDED_ALLDAY_EVENT_HEIGHT
					: mAlldayHeight;
		}
		...
		mMoreAlldayEventsAnimator = ObjectAnimator.ofInt(this, "moreAllDayEventsTextAlpha",
				mShowAllAllDayEvents ? MORE_EVENTS_MAX_ALPHA : 0, mShowAllAllDayEvents ? 0 : MORE_EVENTS_MAX_ALPHA);
		...
		if (mAlldayEventAnimator != null) {
			...
			mAlldayEventAnimator.setStartDelay(mShowAllAllDayEvents ? ANIMATION_SECONDARY_DURATION : 0);
	...
	private ObjectAnimator getAllDayAnimator() {
		...
		int desiredHeight = mShowAllAllDayEvents ? maxADHeight : (int) (MAX_UNEXPANDED_ALLDAY_HEIGHT
				- MIN_UNEXPANDED_ALLDAY_EVENT_HEIGHT - 1);
	...
	private void doSingleTapUp(MotionEvent ev) {
		...
		if (pressedSelected && mSavedClickedEvent == null) {
			...
			mSelectionMode = SELECTION_SELECTED;
			mController.sendEventRelatedEventWithExtra(this, EventType.CREATE_EVENT, -1, getSelectedTimeInMillis(), 0,
			...
			long clearDelay = (CLICK_DISPLAY_DURATION + mOnDownDelay) - (System.currentTimeMillis() - mDownTouchTime);
			if (clearDelay > 0) {
	...
	private void doScroll(MotionEvent e1, MotionEvent e2, float deltaX, float deltaY) {
		...
		if (mRecalCenterHour) {
			...
			// points
			mGestureCenterHour = (mViewStartY + focusY - DAY_HEADER_HEIGHT - mAlldayHeight) / (mCellHeight + DAY_GAP);
		...
		if ((mTouchMode & TOUCH_MODE_VSCROLL) != 0) {
			...
			mViewStartY = (int) ((mGestureCenterHour * (mCellHeight + DAY_GAP)) - focusY + DAY_HEADER_HEIGHT + mAlldayHeight);
	...
	private void doFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
		...
		mScrolling = true;
		mScroller.fling(0 /* startX */, mViewStartY /* startY */, 0 /* velocityX */, (int) -velocityY, 0 /* minX */,
				0 /* maxX */, 0 /* minY */, mMaxViewStartY /* maxY */, OVERFLING_DISTANCE, OVERFLING_DISTANCE);
	...
	public boolean onScaleBegin(ScaleGestureDetector detector) {
		...
		if (DEBUG_SCALING) {
			...
			Log.d(TAG,
					"onScaleBegin: mGestureCenterHour:" + mGestureCenterHour + "\tViewStartHour: " + ViewStartHour
							+ "\tmViewStartY:" + mViewStartY + "\tmCellHeight:" + mCellHeight + " SpanY:"
	...
	public boolean onScale(ScaleGestureDetector detector) {
		...
		if (DEBUG_SCALING) {
			float ViewStartHour = mViewStartY / (float) (mCellHeight + DAY_GAP);
			Log.d(TAG,
					"onScale: mGestureCenterHour:" + mGestureCenterHour + "\tViewStartHour: " + ViewStartHour
							+ "\tmViewStartY:" + mViewStartY + "\tmCellHeight:" + mCellHeight + " SpanY:"
							+ detector.getCurrentSpanY());
		}
		...
		if (mViewStartY < 0) {
			mViewStartY = 0;
			mGestureCenterHour = (mViewStartY + gestureCenterInPixels) / (float) (mCellHeight + DAY_GAP);
		} else if (mViewStartY > mMaxViewStartY) {
			mViewStartY = mMaxViewStartY;
			mGestureCenterHour = (mViewStartY + gestureCenterInPixels) / (float) (mCellHeight + DAY_GAP);
		}
	...
	private class ContextMenuHandler implements MenuItem.OnMenuItemClickListener {
		...
		public boolean onMenuItemClick(MenuItem item) {
			...
			case MENU_EVENT_VIEW: {
				if (mSelectedEvent != null) {
					mController.sendEventRelatedEvent(this, EventType.VIEW_EVENT_DETAILS, mSelectedEvent.id,
							mSelectedEvent.startMillis, mSelectedEvent.endMillis, 0, 0, -1);
				}
				break;
			}
			case MENU_EVENT_EDIT: {
				if (mSelectedEvent != null) {
					mController.sendEventRelatedEvent(this, EventType.EDIT_EVENT, mSelectedEvent.id,
							mSelectedEvent.startMillis, mSelectedEvent.endMillis, 0, 0, -1);
				}
				break;
			}
			case MENU_DAY: {
				mController.sendEvent(this, EventType.GO_TO, getSelectedTime(), null, -1, ViewType.DAY);
				break;
			}
			case MENU_AGENDA: {
				mController.sendEvent(this, EventType.GO_TO, getSelectedTime(), null, -1, ViewType.AGENDA);
				break;
			}
			case MENU_EVENT_CREATE: {
				long startMillis = getSelectedTimeInMillis();
				long endMillis = startMillis + DateUtils.HOUR_IN_MILLIS;
				mController.sendEventRelatedEvent(this, EventType.CREATE_EVENT, -1, startMillis, endMillis, 0, 0, -1);
				break;
			}
			case MENU_EVENT_DELETE: {
				if (mSelectedEvent != null) {
					...
					long end = selectedEvent.endMillis;
					long id = selectedEvent.id;
					mController.sendEventRelatedEvent(this, EventType.DELETE_EVENT, id, begin, end, 0, 0, -1);
				}
				break;
			}
			default: {
	...
	private static int getEventAccessLevel(Context context, Event e) {
		...
		Cursor cursor = cr.query(ContentUris.withAppendedId(Events.CONTENT_URI, e.id),
				new String[] { Events.CALENDAR_ID }, null /* selection */, null /* selectionArgs */, null /* sort */);
	...
	private boolean setSelectionFromPosition(int x, final int y, boolean keepOldSelection) {
		...
		int day = mNumDays - (x / (mCellWidth + DAY_GAP)) - 1;
		if (day < 0) {
			day = 0;
		}
		day += mFirstJulianDay;
		...
		if (y < mFirstCell) {
			...
			if (adjustedY < mFirstHourOffset) {
				...
				setSelectedHour(mSelectionHour + (adjustedY - mFirstHourOffset) / (mCellHeight + HOUR_GAP));
			}
	...
	private void findSelectedEvent(int x, int y) {
		...
		int numEvents = events.size();
		int right = computeDayRightPosition(mSelectionDay - mFirstJulianDay);
		int top = 0;
		setSelectedEvent(null);
		...
		if (mSelectionAllday) {
			...
			for (int i = 0; i < numEvents; i++) {
				Event event = events.get(i);
				if (!event.drawAsAllday() || (!mShowAllAllDayEvents && event.getColumn() >= maxUnexpandedColumn)) {
					// Don't check non-allday events or events that aren't shown
					continue;
				}

				if (event.startDay <= mSelectionDay && event.endDay >= mSelectionDay) {
					float numRectangles = mShowAllAllDayEvents ? mMaxAlldayEvents : mMaxUnexpandedAlldayEventCount;
		...
		for (int i = 0; i < numEvents; i++) {
			...
			if (!geometry.computeEventRect(date, right - cellWidth, top, cellWidth, event)) {
				continue;
			}
	...
	public void cleanup() {
		...
		Utils.setSharedPreference(mContext, GeneralPreferences.KEY_DEFAULT_CELL_HEIGHT, mCellHeight);
					...
					public void onClick(DialogInterface dialog, int which) {
						if (which == 0) {
							...
							mController.sendEventRelatedEventWithExtra(this, EventType.CREATE_EVENT, -1,
									getSelectedTimeInMillis(), 0, -1, -1, extraLong, -1);
						}
					}
	...
	private long calculateDuration(float delta, float width, float velocity) {
		...
		if (DEBUG) {
			Log.e(TAG, "halfScreenSize:" + halfScreenSize + " delta:" + delta + " distanceRatio:" + distanceRatio
					+ " distance:" + distance + " velocity:" + velocity + " duration:" + duration
					+ " distanceInfluenceForSnapDuration:" + distanceInfluenceForSnapDuration);
		}
		return duration;
	}
