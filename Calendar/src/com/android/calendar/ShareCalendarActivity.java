...
/**
 * Handles requests for sharing calendar events This activity returns a vcs
 * formatted file
 */
public class ShareCalendarActivity extends FragmentActivity implements CalendarUtils.ShareEventListener {
	...
	private HashSet<CalendarUtils.Triple<Long, Long, Long>> mShareEventsList = new HashSet<CalendarUtils.Triple<Long, Long, Long>>();
	...
	public void onEventRemoval(long eventId) {
		...
		CalendarUtils.Triple<Long, Long, Long> eventInfo = new CalendarUtils.Triple<Long, Long, Long>(eventId, 0l, 0l);
	...
	public void updateSubtitle() {
		...
		String subtitle;
		// workaround for Android's poor pluralization treatment of 'zero'
		// quantity in English
		if (listSize == 0) {
			subtitle = mNoSelectionMsg;
		} else {
			subtitle = mResources.getQuantityString(R.plurals.events_selected, listSize, listSize);
		}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		String title = mShouldSelectSingleEvent ? mResources.getQuantityString(R.plurals.select_events_to_share, 1)
	...
	private void generateEventData() {
		for (CalendarUtils.Triple<Long, Long, Long> event : mShareEventsList) {
			...
			// EventInfoFragment just serves as a data fragment and is
			// initialized with
			// default arguments for parameters that don't affect model loading
			EventInfoFragment eif = new EventInfoFragment(this, eventId, eventStartMillis, eventEndMillis,
					CalendarContract.Attendees.ATTENDEE_STATUS_NONE, false, 0, null);
	...
	// generates the vcs files if the data for selected events has been
	...
	private void evalIfComplete() {
			...
			for (EventInfoFragment event : mEventDataFragments) {
				try {
					...
					if (filePrefix == null || filePrefix.length() < 3) {
						// default to a generic filename if event title doesn't
						// qualify
						// prefix length constraint is imposed by
						// File#createTempFile
						filePrefix = "invite";
					}
		...
		} else if (mEventDataFragments.size() < 1) { // if no events have been
														// selected
			setResultAndFinish(false);
		}
	}
