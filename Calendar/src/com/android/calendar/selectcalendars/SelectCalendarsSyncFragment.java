...
public class SelectCalendarsSyncFragment extends ListFragment implements View.OnClickListener,
		LoaderManager.LoaderCallbacks<Cursor> {

	private static final String TAG = "SelectCalendarSync";

	private static final String COLLATE_NOCASE = " COLLATE NOCASE";
	private static final String SELECTION = Calendars.ACCOUNT_NAME + "=? AND " + Calendars.ACCOUNT_TYPE + "=?";
	// is primary lets us sort the user's main calendar to the top of the list
	private static final String IS_PRIMARY = "\"primary\"";
	private static final String SORT_ORDER = IS_PRIMARY + " DESC," + Calendars.CALENDAR_DISPLAY_NAME + COLLATE_NOCASE;

	private static final String[] PROJECTION = new String[] { Calendars._ID, Calendars.CALENDAR_DISPLAY_NAME,
			Calendars.CALENDAR_COLOR, Calendars.SYNC_EVENTS, Calendars.ACCOUNT_NAME, Calendars.ACCOUNT_TYPE,
	...
	public void onResume() {
		...
		if (!ContentResolver.getMasterSyncAutomatically()
			...
			// Start a background sync to get the list of calendars from the
			// server.
			Utils.startCalendarMetafeedSync(mAccount);
			getActivity().getContentResolver().registerContentObserver(Calendars.CONTENT_URI, true, mCalendarsObserver);
		}
	}
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args) {
		...
		return new CursorLoader(getActivity(), Calendars.CONTENT_URI, PROJECTION, SELECTION, mArgs, SORT_ORDER);
	}
