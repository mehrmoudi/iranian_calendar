...
package com.android.colorpickerstock;
...
/**
 * A color picker custom view which creates an grid of color squares. The number
 * of squares per row (and the padding between the squares) is determined by the
 * user.
 */
public class ColorPickerPalette extends TableLayout {
	...
	/**
	 * Initialize the size, columns, and listener. Size should be a pre-defined
	 * size (SIZE_LARGE or SIZE_SMALL) from ColorPickerDialogFragment.
	 */
	...
	/**
	 * Appends a swatch to the end of the row for even-numbered rows (starting
	 * with row 0), to the beginning of a row for odd-numbered rows.
	 */
	...
	/**
	 * Add a content description to the specified swatch view. Because the
	 * colors get added in a snaking form, every other row will need to
	 * compensate for the fact that the colors are added in an opposite
	 * direction from their left->right/top->bottom order, which is how the
	 * system will arrange them for accessibility purposes.
	 */
	...
	private ColorPickerSwatch createColorSwatch(int color, int selectedColor) {
		ColorPickerSwatch view = new ColorPickerSwatch(getContext(), color, color == selectedColor,
