...
/**
 * The primary layout to hold the circular picker, and the am/pm buttons. This
 * view well measure itself to end up as a square. It also handles touches to be
 * passed in to views that need to know when they'd been touched.
 */
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
	...
	public void initialize(Context context, HapticFeedbackController hapticFeedbackController, int initialHoursOfDay,
		...
		for (int i = 0; i < 12; i++) {
			hoursTexts[i] = Jalali.persianDigits(is24HourMode ? String.format("%02d", hours_24[i]) : String.format(
					"%d", hours[i]));
			innerHoursTexts[i] = Jalali.persianDigits(String.format("%d", hours[i]));
			minutesTexts[i] = Jalali.persianDigits(String.format("%02d", minutes[i]));
		}
		...
		mHourRadialSelectorView.initialize(context, mHideAmPm, is24HourMode, true, hourDegrees,
